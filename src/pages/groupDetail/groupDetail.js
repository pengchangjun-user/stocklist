...
const GroupDetail = (props) => {
    ...
    const handleData = (allPeriod, idNumbers, lastPeriod) => {
        ...
        for (const key in allData) {
            if (allData.hasOwnProperty(key)) {
                ...
                allData[key].forEach(item => {
                    if(idNumbers.includes(item.id_number)) {
                        ...
                        allGroupData[key].push(item)
                    }
                })
                allGroupData[key] = allGroupData[key].sort(sortHolderAmount)
            }
        }
        console.log("allGroupData", allGroupData)
        // 最新一期的数据
        let lastData = allGroupData[lastPeriod]
        let members = []
        let groupInfo = {
            amount: 0,
            ratio: 0
        ...
        for(let i = 0, iLen = lastData.length; i < iLen; ++i) {
            let item = lastData[i]
            groupInfo.amount += item.holder_amount
            groupInfo.ratio += item.holder_ratio
            members.push({
                key: i,
        ...
        groupInfo.ratio = groupInfo.ratio ? parseFloat((groupInfo.ratio * 1).toFixed(2)) : "0.00"
        setMembersArr(members)
        setGroupInfo(groupInfo)
        // 图表数据
        if (!(JSON.stringify(allGroupData) == "{}")) {
            let chartData = {
                holder_amount: [],
                ratio: []
            }
            ...
            for (let i = 0; i < periods.length; i++) {
                let item = periods[i]
                let dataItem = allGroupData[item]
                // 保证这一期里面有数据
                if (dataItem.length > 0) {
                    ...
                    dataSource.push({
                        ...
                        // 如果是t3, 那么取normal_amount， 否则取holder_amount - credit_amount
    ...
    useEffect(() => {
        ...
        if (params) {
            ...
            handleData(allPeriod, params.idNumbers, params.period)
        }
    }, [])
    ...
    return (
        <div className="group-detail">
            ...
            <div className="detail-title">{groupName}</div>
            <div className="column">
                <div className="col">组内合计持股数量(股): <span>{formatNumber(groupInfo.amount)}</span></div>
                <div className="col">组内合计持股比例(%)：<span>{groupInfo.ratio || '--' }</span></div>
            </div>
            <div className="detail-title">组内成员持股情况</div>
